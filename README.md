...
Set `MCP_SERVER_URL` to the deployed API MCP endpoint before starting the bot. The bot uses Groq as its primary chat provider and Gemini as its fallback. Webhook mode also requires `WEBHOOK_BASE_URL`, `TELEGRAM_WEBHOOK_PATH`, and `TELEGRAM_WEBHOOK_SECRET`.
