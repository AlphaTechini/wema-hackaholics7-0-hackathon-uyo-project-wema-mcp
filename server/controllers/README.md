...
- To find the balance lookup logic visit [getBalance.ts](getBalance.ts).
- To find account switch PIN verification visit [verifyAccountPin.ts](verifyAccountPin.ts).
- To find Telegram ban status lookup visit [getTelegramSecurityStatus.ts](getTelegramSecurityStatus.ts).
